...
async def process_user_message(tg_id: int, user_text: str) -> str:
    ...
    system_instruction = (
        ...
        "1. Если пользователь просит написать через X минут, прибавь их к текущему времени (UTC). "
        "2. Формат 'next_contact_time' ДОЛЖЕН БЫТЬ строго ISO: YYYY-MM-DDTHH:MM:SS. "
        "3. Если вызываешь update_ai_memory, всегда подтверждай это в тексте ответа."
    )
